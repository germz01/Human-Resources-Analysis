...
\begin{figure}[H]
  ...
  \caption{Dendrogramma per metodo 'centroid' e distanza euclidea, con esempio di taglio con 4 clusters, rappresentati dai differenti colori.}
...
Dalle analisi condotte con le tre metodologie di clustering abbiamo rilevato che con la metodologia del DBSCAN e dello Hierarchical non si riescono a ricavare delle informazioni rilevanti ai fini della ricerca in quanto si ottengono raggruppamenti con la presenza di un cluster in cui vi sono la quasi totalità dei dipendenti del dataset.
La divisione in cluster ottenuta con il K-means ha permesso una suddivisione migliore dei dipendenti, con l'analisi mostrata nel relativo paragrafo.
La silhouette media per il K-means risulta abbastanza alta, con un valore pari 0.57, superiore ai valori ottenuti con il metodo hierachical, per il corrispondente taglio dei dendrogrammi in 4 cluster. Quindi per questo dataset la metodologia del K-means è risultata la più efficace.
