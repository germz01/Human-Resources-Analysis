...
p<-p+ggtitle("Association Rules for MinSupp=20%")
p<-p+scale_size(range=c(5,20),trans="identity")
...
p<-p+geom_smooth(method="lm",size=1)#,col="black")
p<-p+geom_vline(xintercept = 1,col="red",size=3,linetype="solid")
p<-p+ggtitle("Association Rules for MinSupp=5%, Supp<20% , Lift>1, Conf>0.90")
p<-p+scale_size(range=c(5,20),trans="identity")
p<-p+scale_color_brewer(palette='Paired')
p<-p+scale_fill_brewer(palette='Paired')
...
p<-p+theme_bw()
#p+annotate("text",x=6,y= 0.85, label="")
p
