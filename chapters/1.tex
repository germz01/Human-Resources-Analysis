...
In tabella \ref{tab:variabili} sono riportate le variabili relative ai dipendenti dell'aziendae la corrispondente tipologia. 
...
Il dataset analizzato è composto da variabili di tipo misto, ovvero sia categoriche che numeriche.
Trasformare le variabili in un'unica tipologia categorica/numerica consente di trattare uniformemente le variabili,
soprattutto per la scelta della distanza da utilizzare nei metodi di clustering. 
Per eseguire le analisi successive è possibile trasformare le variabili utilizzando differenti strategie, che 
consentano di ottenere diverse tipologie di datasets:
\begin{enumerate}
\item dataset con tutte le variabili numeriche
\item dataset con tutte le variabili categoriche
\item data set con variabili miste  
\end{enumerate}
Nel primo caso è necessario trasformare le variabili categoriche in variabili numeriche. Per le variabili categoriche binarie è sufficiente
assegnare valori 0 o 1 alle variabili. Nel caso di un numero maggiore di categorie, come per la variabile \textit{Department} è possibile
operare una binarizzazione, considerando le categorie della variabile Department come nuovi attributi binari.
Per ottenere un dataset di sole variabili categoriche è necessario definire degli intervalli per ciascuna variabile numerica.
In questo caso non è necessario una binarizzazione.
Infine si può trattare il dataset considerando variabili di tipo misto, utilizzando metodi ad-hoc.
