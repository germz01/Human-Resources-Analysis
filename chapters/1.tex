...
\begin{figure}[ht!]
  ...
  \begin{subfigure}{0.3\textwidth}
    \resizebox{\textwidth}{!}{
      \includegraphics{images/newplottedvariables/sales.pdf}
    }
    \caption{}
    \label{fig:sales}
  \end{subfigure}
  \begin{subtable}{0.3\textwidth}
    \resizebox{\textwidth}{!}{
      \begin{tabular}{| c | c | c | c |} \hline
        ...
        product\_mng  & 704 & 198 & 902  \\ \hline
  ...
  \begin{subfigure}{0.4\textwidth}
    \resizebox{\textwidth}{!}{
      \includegraphics{images/newplottedvariables/average_montly_hours.pdf}
    }
    \caption{}
  ...
  \begin{subtable}{0.4\textwidth}
    ...
    \label{tab:avg_work}
  \end{subtable}
  \caption{Distribuzione relativa alla variabile \textit{Departments} (\ref{fig:sales}), e relativa
  distribuzione dei dipendenti per ogni dipartimento (\ref{tab:sales}), distribuzione relativa alla variabile
  \textit{Average Montly Hours} (\ref{fig:average_montly_hours}) e relativa tabella (\ref{tab:avg_work}).}
  \label{fig:dist_sales}
\end{figure}
\begin{figure}[hb!]
  \centering
  \begin{subfigure}{0.4\textwidth}
    \resizebox{\textwidth}{!}{
      \includegraphics{images/newplottedvariables/work_accident.pdf}
    }
    \caption{}
  ...
  \begin{subfigure}{0.4\textwidth}
    \resizebox{\textwidth}{!}{
      \includegraphics{images/newplottedvariables/promotion_last_5_years.pdf}
    }
    \caption{}
    \label{fig:promotion}
  \end{subfigure}
  \begin{subfigure}{0.4\textwidth}
    \resizebox{\textwidth}{!}{
      \includegraphics{images/newplottedvariables/salary.pdf}
    }
    \caption{}
    \label{fig:salary}
  \end{subfigure}
  \begin{subtable}{0.4\textwidth}
    ...
    \label{tab:salary}
  \end{subtable}
  \caption{Distribuzioni relative alla variabile \textit{Work Accident} (\ref{fig:work_accident}), alla
  variabile \textit{Promotion Last 5 Years} (\ref{fig:promotion}), alla variabile \textit{Salary}
  (\ref{fig:salary}) e alla relativa distribuzione del salario per dipendente (\ref{tab:salary}).}
  \label{fig:dist_work_promotion}
\end{figure}
...
promozione negli ultimi $5$ anni, a parte $19$ impiegati che è stata promossa (circa il 0.5\% ossia circa il
0,13\% dei dipendenti totali), praticamente impercettibili alla vista nel grafico. Degli impiegati rimasti, in
$300$ hanno ottenuto una promozione su $11428$ (circa il 2,62\% di quelli rimasti e circa il
...
Le percentuali che vengono indicate in Tabella \ref{tab:salary} non sono in base alla totalità dei dipendenti ma
riguardano solo il tipo di dipendente definito dalla colonna di appartenenza. In Figura
\ref{fig:average_montly_hours} troviamo il rapporto tra dipendenti ancora in azienda e non, e il numero delle
ore di lavoro in media. Analizzando Figura \ref{fig:num_proj}, l'informazione chiave che
risulta da questa distribuzione è che la totalità dei dipendenti che hanno fatto $7$ progetti hanno lasciato
l'azienda, questo è quindi sicuramente uno dei fattori per cui i dipendenti potrebbero lasciare l'azienda.
L'altro valore che risalta è i dipendenti che hanno fatto solo $2$ progetti, in numero di $1567$, ovvero quasi
il $44\%$ di quelli che hanno lasciato l'azienda. Di questi dovremo capire quali motivazioni li hanno portati a
lasciare l'azienda, se il poco carico di lavoro o altre circostanze lavorative.
\begin{figure}[ht!]
  \centering
  \begin{subfigure}{0.4\textwidth}
    \resizebox{\textwidth}{!}{
      \includegraphics{images/newplottedvariables/number_project.pdf}
    }
    \caption{}
    \label{fig:num_proj}
  \end{subfigure}
  \begin{subtable}{0.3\textwidth}
    ...
    \label{tab:num_proj}
  \end{subtable}
  \caption{Distribuzione relativa alla variabile \textit{Number Project} (\ref{fig:num_proj}), e relative
  informazioni riguardo al numero di dipendenti per numero di progetti (\ref{tab:num_proj}).}
  \label{fig:dist_num_proj}
\end{figure}
Proseguiamo poi mostrando il tempo di impiego di lavoro nell'azienda, in Figura
\ref{fig:t_spend_comp}.
\begin{figure}[hb!]
  \centering
  \begin{subfigure}{0.4\textwidth}
    \resizebox{\textwidth}{!}{
      \includegraphics{images/newplottedvariables/time_spend_company.pdf}
    }
    \caption{}
  ...
  \begin{subfigure}{0.4\textwidth}
    \resizebox{\textwidth}{!}{
      \includegraphics{images/newplottedvariables/satisfaction_level.pdf}
    }
    \caption{}
    \label{fig:sat_lev}
  \end{subfigure}
  \begin{subfigure}{0.4\textwidth}
    \resizebox{\textwidth}{!}{
      \includegraphics{images/newplottedvariables/last_evaluation.pdf}
    }
    \caption{}
    \label{fig:last_ev}
  \end{subfigure}
  ...
  \begin{subtable}{0.4\textwidth}
    ...
    \label{tab:last_ev}
  \end{subtable}
  \caption{Distribuzione relativa alla variabile \textit{Time Spend Company} (\ref{fig:t_spend_comp}), e relative
  informazioni sui dipendenti (\ref{tab:t_spend_comp}), distribuzioni relative alle variabili
  \textit{Satisfaction Level} (\ref{fig:sat_lev}) e \textit{Last Evaluation} (\ref{fig:last_ev}), e relative
  tabelle (\ref{tab:sat_lev} e \ref{tab:last_ev}).}
  \label{fig:dist}
\end{figure}
Si può rilevare un fattore importante, dal settimo anno in azienda non abbiamo dipendenti che hanno lasciato
l'azienda. Inoltre la maggior parte del numero di dipendenti che hanno lasciato l'azienda lo abbiamo in un range
dai $3$ ai $5$ anni come fattore critico, con un massimo di $1587$ dipendenti, ovvero quasi il $44.4\%$ di
quelli che lasciano l'azienda, nel terzo anno di lavoro. A questo punto è giusto analizzare il livello di
soddisfazione dei dipendenti presente e quello della ultima valutazione, rappresentati in Figura
\ref{fig:sat_lev} e Figura \ref{fig:last_ev}.
